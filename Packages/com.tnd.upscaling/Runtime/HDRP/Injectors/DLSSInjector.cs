...
namespace UnityEngine.Rendering.HighDefinition.NVIDIA
{
    ...
    public class DLSSContext: UpscalerContext
    {
        ...
        public Vector2 GetJitterOffset(int taaFrameIndex, int renderWidth, int displayWidth)
        {
            if (ActiveUpscaler != null)
            {
                return ActiveUpscaler.GetJitterOffset(taaFrameIndex, renderWidth, displayWidth);
            }

            return new Vector2(
                HaltonSequence.Get(taaFrameIndex, 2) - 0.5f,
                HaltonSequence.Get(taaFrameIndex, 3) - 0.5f);
        }
